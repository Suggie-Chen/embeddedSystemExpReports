...
//	# error "Not Implemented!"
	...
	if(board_getc_ready() ){
		...
		gett = board_getc();//get
		fifo_push(&sendfifo,gett);
		command[cmd_idx] = gett;
		cmd_idx++;
	}
		if(gett=='\r'){
			char *inputs[3];
			int pd= split_cmdline(command,cmd_idx,inputs);
			led_cmd_exec(pd,inputs);

			for(cmd_idx = 0; cmd_idx < COMMAND_LEN_MAX; cmd_idx++)
				command[cmd_idx]='\0';
			cmd_idx=0;
