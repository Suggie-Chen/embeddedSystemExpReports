...
void key_process() {
	...
	if (key_states[K4].output_trig_up) {
		...
		unsigned int presstime = ks->trig_up_ms - ks->trig_down_ms;
		//long push
		if (PRESS_TIME_MS_LONG < presstime) {
			HAL_CPU_Reset();
		}
	}
	...
	if (key_states[K6].output_trig_down) {
		...
		while(1){
			unsigned int presstime = HAL_GetTick() - ks->trig_down_ms;
			//long push
			if (PRESS_TIME_MS_LONG * 4 < presstime) {
				HAL_Led_write(4, 0);
				//0 -> on  1 -> off
			}else if (PRESS_TIME_MS_LONG * 3 < presstime) {
				HAL_Led_write(3, 0);
			}else if (PRESS_TIME_MS_LONG * 2 < presstime) {
				HAL_Led_write(2, 0);
			}else if (PRESS_TIME_MS_LONG * 1 < presstime) {
				HAL_Led_write(1, 0);
			}
			key_scan(K6);
			if(ks->output_trig_up){
