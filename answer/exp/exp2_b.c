...
void key_process() {
	...
	if (key_states[K3].output_trig_down){
		k5_push=0;
		led_flash=0;
		HAL_GPIO_WritePin(gpio_configs[LED6].Port,gpio_configs[LED6].Pin,(GPIO_PinState)(1));
		HAL_GPIO_WritePin(gpio_configs[LED7].Port,gpio_configs[LED7].Pin,(GPIO_PinState)(1));
		HAL_GPIO_WritePin(gpio_configs[LED8].Port,gpio_configs[LED8].Pin,(GPIO_PinState)(1));
		HAL_GPIO_WritePin(gpio_configs[LED9].Port,gpio_configs[LED9].Pin,(GPIO_PinState)(1));
		HAL_CPU_Sleep();
	}
	
	if (key_states[K4].output_level==0){
		k5_push=0;
		int duration=HAL_GetTick()-key_states[K4].trig_down_ms;
		if (duration>PRESS_TIME_MS_HOLD){
			led_flash=0;
			HAL_CPU_Reset();
		}
	}
	...
	if (key_states[K5].output_trig_up){
		int t=key_states[K5].trig_up_ms-key_states[K5].trig_down_ms;
		if (t>PRESS_TIME_MS_SHORT&&t<PRESS_TIME_MS_LONG){
			if (k5_push==1){
				k5_push=0;
				led_flash=1;
			}
	...
	if (key_states[K6].output_level==0){
		led_flash=0;
		k5_push=0;
		int now=HAL_GetTick();
		int duration=now-key_states[K6].trig_down_ms;
		led_on=0;
		if (duration<PRESS_TIME_MS_HOLD){
			HAL_GPIO_WritePin(gpio_configs[LED6].Port,gpio_configs[LED6].Pin,(GPIO_PinState)(1));
			HAL_GPIO_WritePin(gpio_configs[LED7].Port,gpio_configs[LED7].Pin,(GPIO_PinState)(1));
		...
		else if (duration>=PRESS_TIME_MS_HOLD&&duration<2*PRESS_TIME_MS_HOLD){
			HAL_GPIO_WritePin(gpio_configs[LED6].Port,gpio_configs[LED6].Pin,(GPIO_PinState)(0));
		}
		else if (duration>=2*PRESS_TIME_MS_HOLD&&duration<3*PRESS_TIME_MS_HOLD){
			HAL_GPIO_WritePin(gpio_configs[LED7].Port,gpio_configs[LED7].Pin,(GPIO_PinState)(0));
		}
		else if (duration>=3*PRESS_TIME_MS_HOLD&&duration<4*PRESS_TIME_MS_HOLD){
			HAL_GPIO_WritePin(gpio_configs[LED8].Port,gpio_configs[LED8].Pin,(GPIO_PinState)(0));
		}
		else if (duration>=4*PRESS_TIME_MS_HOLD&&duration<5*PRESS_TIME_MS_HOLD){
			HAL_GPIO_WritePin(gpio_configs[LED9].Port,gpio_configs[LED9].Pin,(GPIO_PinState)(0));
		}
	}
}
