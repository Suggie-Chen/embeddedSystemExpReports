...
char* shell_readline(struct uart_shell *shell)  {
	...
	while (1) {
		...
		if(ch == 0x1b){
			shell->stat = WAIT_SPEC_KEY;
				...
				if(ch == 0x5b){
					ch = shell-> _getchar();
					if(ch == 0x41){
						//todo: to the previous cmd
						if(shell->history_count > 0 && shell->current_history != 0){
							//need copy history to cmd_line??
							ush_printf(shell, "%s%s", SHELL_PROMPT, shell->cmd_history[--shell->current_history]);							
						}
					}
					if(ch == 0x42){
						//todo: to the next cmd
						if(shell->history_count > 0&& shell->current_history < shell->history_count)
							ush_printf(shell, "%s%s", SHELL_PROMPT, shell->cmd_history[++shell->current_history]); 
					}
					if(ch == 0x43){
						//to the right letter
						if(shell->line_position> 0&& shell->line_curpos < shell->line_position && shell->line_position < CMD_SIZE-1)
						{
							
							char c = shell->line[shell->line_curpos];
					...
					if(ch == 0x44){
						//to the left letter
						if(shell->line_position> 0&& shell->line_curpos >= 0 && shell->line_position < CMD_SIZE-1)
						{
		...
		if (ch == 0x7f || ch == 0x08) {
			//TODO: display the modified command 
			if(shell->line_position > 0){
